...
> **Vault Integration**: Ensure Claude Agent Teams has explicit access to `C:\Users\Zartc\Vault\` to read/write `decisions/`, `journal/`, and `spec/`.
...
3. **Vault as the Source of Truth**: Agents will use `C:\Users\Zartc\Vault\` to communicate asynchronously:
