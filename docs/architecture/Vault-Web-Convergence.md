...
- **Purpose**: A local workspace GUI that bridges the gap between the code execution layers and the documentation Vault (`C:\VSCode\Vault`).
...
`AGENTS.md` was deprecated to eliminate unconstrained, redundant system prompts in the global codebase. The AI agents are now instructed to conduct all inter-process communications (IPC), handoffs, and architectural logging dynamically via the user's local Obsidian Vault (`C:\VSCode\Vault`).
...
1. **Vault-Web**:
   ...
   - Ensure the Vault path (`C:\VSCode\Vault`) matches the configured path in `Vault-Web/src/lib/vault.ts`.
