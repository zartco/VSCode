...
export async function ingestRoutes(app) {
    ...
    `).run(a.id, a.source, a.name, a.status, a.sessionId, a.cwd, a.startedAt, a.lastSeenAt, JSON.stringify(a.metadata));
    ...
    app.post('/ingest/event', async (req, reply) => {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(e.id, e.agentId, e.taskId ?? null, e.type, e.timestamp, JSON.stringify(e.payload));
        if (result.changes > 0) {
    ...
    app.post('/ingest/task', async (req, reply) => {
        const t = req.body;
        getDb().prepare(`
      INSERT OR REPLACE INTO tasks (id, agent_id, parent_task_id, description, status, started_at, completed_at, metadata)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(t.id, t.agentId, t.parentTaskId ?? null, t.description, t.status, t.startedAt, t.completedAt ?? null, JSON.stringify(t.metadata));
        broadcast({ kind: 'task_upsert', task: t });
        return reply.status(204).send();
    });
    // Ingest Obsidian vault modifications
    app.post('/ingest/vault', async (req, reply) => {
