import type { TaskNode } from '@contracts'
import './TaskTree.css'
...
function TaskRow({ task, depth, allTasks }: TaskRowProps) {
  ...
  return (
    <>
      ...
      {children.map((child) => (
        <TaskRow key={child.id} task={child} depth={depth + 1} allTasks={allTasks} />
      ))}
    </>
  )
}

export function TaskTree({ tasks, selectedAgentId }: Props) {
