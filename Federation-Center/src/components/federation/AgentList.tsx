...
import type { AgentNode, AgentEvent } from "@/contracts/types";
...
function SourceBadge({ source }: { source: AgentNode["source"] }) {
  const color =
    source === "claude" ? "var(--accent-orange)" : "var(--accent-matrix)";
  return (
...
export function AgentList() {
  ...
  const renderAgentList = (list: AgentNode[]) => (
    <ul className="agent-cards">
      {list.map((agent) => {
        ...
        const isOrch =
          agent.source === "antigravity" && agent.id === latestAntigravity?.id;
        return (
          <li key={agent.id}>
            <button
              className={`agent-card ${
                selectedAgentId === agent.id ? "agent-card--selected" : ""
              } ${isOrch ? "agent-card--orchestrator" : ""}`}
              onClick={() => handleClick(agent.id)}
              aria-pressed={selectedAgentId === agent.id}
            >
              {isOrch && (
                <div className="orchestrator-badge">★ PRIMARY ORCHESTRATOR</div>
              )}
              <div className="agent-card__header">
                ...
                <div className="agent-card__header-right">
                  {eventCount > 0 && (
                    <span
                      className="agent-card__event-count"
  ...
  return (
    <aside className="agent-list">
      ...
      <h2 className="panel-title" style={{ marginTop: "20px" }}>
        Agent History
      </h2>
      {historicalAgents.length > 0 ? (
        renderAgentList(historicalAgents)
      ) : (
        <div className="empty-state" style={{ padding: "16px", color: "var(--text-muted)" }}>
          No history.
        </div>
