 ...
 */
const getSimpleTypeUnion = (v) => {
    if (!('anyOf' in v) || !Array.isArray(v.anyOf))
        return null;
    const types = [];
...
export const getDescription = (v, depth = 0) => {
    ...
    const parts = Object.entries(v)
        ...
        if (depth <= 1 && k === 'anyOf' && getEnumChoices(v)?.type === 'string_enum')
            return false;
        // don't show anyOf: [...] for simple type unions - we'll render it as "type: X or Y" instead
        if (k === 'anyOf' && simpleTypeUnion)
            return false;
        return true;
    })
        .sort(([a], [b]) => {
...
export const getAllowedSchemas = (schema) => {
    if (!schema)
        return [];
 ...
 */
export function toJsonSchema(input, dependencies) {
    try {
        ...
        if (typeof input?.toJsonSchema === 'function') {
            return { success: true, value: input.toJsonSchema() };
        }
...
const getJsonSchemaConverters = (dependencies) => {
    return {
        zod: (input) => {
            ...
            return zodV3ToJsonSchema(input);
        },
        arktype: (input) => {
...
export function looksJsonSchemaable(value) {
