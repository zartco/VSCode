...
* **Vault IPC Delegation:** Since persistent custom subagents are restricted, you will use temporary session subagents. All finalized handoffs, state transfers, and instructions for Jules MUST be written to the local filesystem at `C:\Users\Zartc\Vault\`.
...
1. **The QA/Reviewer Subagent**
   ...
   * **Prompt Directive:** "Execute all tests in your isolated branch. Do not execute Git commits. If tests fail, attempt to resolve them within your scope. Report blockers via send_message. When complete, write a structured summary to C:\Users\Zartc\Vault\handoffs\."
...
2. **The Knowledge Steward**
   ...
   * **Prompt Directive:** "Review recent execution logs. Format your findings as concise markdown mapping code to CS concepts. Write directly into C:\Users\Zartc\Vault\decisions\. Alert the parent agent via send_message when finished."
...
Establish connection with the Inter-Process Communication (IPC) hub at `C:\Users\Zartc\Vault\`. Use your file tools to securely process the contents of `C:\VSCode\Scaffold` and organize them into the Vault taxonomy:
...
* **Root (`C:\Users\Zartc\Vault\`):** Create a `README.md` containing the **Vault Root Directory Guide**.
...
2. Write this file to `C:\Users\Zartc\Vault\handoffs\ready-for-jules-public-preview.md`. This physical file acts as the trigger for the local Jules CLI to execute the final version control mutations.
